...
### HSK learning features

This workspace now includes two HSK-focused features powered by `ref.json`:

- Flashcards with active recall and spaced repetition (Anki-style grading):
  - Route: `/dashboard/flashcards`
  - Persisted state with Zustand + `persist`
  - Due-card filtering and scheduling with tested SRS logic
- Character map explorer:
  - Route: `/dashboard/character-map`
  - Search by character, pinyin, word, or meaning
  - Built from the same vocabulary dataset
